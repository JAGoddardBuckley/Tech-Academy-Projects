#Python ver. 3.6.2
#Jenn Buckley Phonebook Demo Drill The Tech Academy
...
# catch clicks on the windows upper-right 'X' to ensure they want to close
def ask_quit(self):
...
def onSelect(self,event):
    #calling the event is the self.lstBox1 widget
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0): # enforce the user to provide both names
        ...
        with conn:
            ...
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                ...
                self.lstBox1.insert(END, var_fullname) # update listbox with the new fullname
...
def onDelete(self):
    var_select = self.lstBox1.get(self.lstBox1.curselection()) # Listbox's selected value
    conn = sqlite3.connect('db_phonebook.db')
    with conn:
        cur = conn.cursor()
...
##    onRefresh(self) # update the listbox of the changes
    try:
        index = self.lstBox1.curselection()[0]
        self.lstBox1.delete(index)
    except IndexError:
        pass
...
def onRefresh(self):
    ...
    with conn:
        ...
        while i < count:
                ...
                for item in varList:
                    self.lstBox1.insert(0,str(item))
                    i = i + 1
    conn.close()
...
def onUpdate(self):
    try:
        var_select = self.lstBox1.curselection()[0] # index of the list selection
        var_value = self.lstBox1.get(var_select) # list selection's text value
    except:
