...
def load_gui(self):
    ...
    self.lbl_info.grid(row = 0, column =2, padx =(0,0), pady=(10,0), sticky ='nw')


    self.txt_fname = tk.Entry(self.master,text = '')
    self.txt_fname.grid(row=1, column=0, rowspan=1, columnspan=2,
                        padx=(30,40), pady=(0,0), sticky = 'new')
    ...
    self.lstBox1 = Listbox(self.master,exportselection=0, yscrollcommand=self.scrollbar1.set)
    self.lstBox1.bind('<<ListboxSelect>>', lambda event: phonebook_func.onSelect(self, event))
    self.scrollbar1.config(command=self.lstBox1.yview)
    self.scrollbar1.grid(row=1, column=5,rowspan=7, columnspan=1, padx=(0,0), pady=(0,0), sticky='nes')
    self.lstBox1.grid(row=1, column=2, rowspan=7, columnspan=3, padx=(0,0), pady=(0,0), sticky = 'nesw')
